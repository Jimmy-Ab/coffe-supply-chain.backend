...
class CoffeeDeliveryAgreementContract extends Contract
{
  constructor()
  {
    super("CoffeeDeliveryAgreementContract");
    this.txId = "";
  }
  ...
  {
    console.log("CoffeeDeliveryAgreementContract");
  }
  ...
  )
  {
    ...
    const coffeeDeliveryAgreementBuffer = CoffeeDeliveryAgreement.from({
      ...
      deliveryLongtitude: deliveryLongtitude,
  ...
  async contractApproval(ctx, id, ectRemark)
  {
    ...
    if (
      _deliveryContract.contractStatus !==
      CoffeeDeliveryAgreementStatus.SIGNED
    )
    {
      throw new Error(`The contract is not on the signing stage`);
    }
  ...
  //contract approval by Ethiopian coffee and tea authority
  /* async contractApproval(ctx, id, ectRemark) {
    this._require(id, "id");
    this._require(ectRemark, "ectRemark");
    if (!(await this._doesDeliveryAgreementExist(ctx.stub, id))) {
    ...
    if (
      ...
      throw new Error(`The contract is not on the approval stage`);
    }
    _deliveryContract.contractStatus = CoffeeDeliveryAgreementStatus.APPROVED;
    _deliveryContract.signedDate = new Date();
    _deliveryContract.ectRemark = ectRemark;
  ...
  _createCK(stub, id)
  {
    return stub.createCompositeKey("CoffeeDeliveryAgreement", [`${id}`]);
  }
